...
def rank_texts_by_keyphrases(texts, keyphrase_score_pairs: List[Tuple[str, int]]):
    processed_texts = [preprocess_text(text) for text in texts]
...
def rank_projects_by_keyphrases(projects, keyphrase_score_pairs: List[Tuple[str, int]]):
    project_texts = [
        f"{project['title']} {project['description']}]" for project in projects
    ]

    # Get rankings for the projects
    ranks, _ = rank_texts_by_keyphrases(project_texts, keyphrase_score_pairs)
...
def rank_experiences_by_keyphrases(
    experiences, keyphrase_score_pairs: List[Tuple[str, int]]
):
    experience_texts = [
    ...
    ranks, _ = rank_texts_by_keyphrases(experience_texts, keyphrase_score_pairs)
